...
if(isset($_POST['debitnote_id']))
{
    ...
    $sql = "SELECT * from debitnotes where debitnote_id = '$debitnote_id' ";
    $result = mysqli_query($dbcon,$sql);
    $row =$result-> fetch_assoc();
    $debitnote_comp_code = $row['debitnote_comp_code'];
    $debitnote_vendor = $row['debitnote_vendor'];
    $debitnote_items = $row['debitnote_items'];
    $inv_items_arr = json_decode($debitnote_items);
    $sql1 = "SELECT * from comprofile where orgid='$debitnote_comp_code' limit 1 ";
    $result1 = mysqli_query($dbcon,$sql1);
    $row1 =$result1-> fetch_assoc();
    //print_r($row1);

    $sql2 = "SELECT * from vendorprofile where vendorid = '$debitnote_vendor' limit 1";
    $result2 = mysqli_query($dbcon,$sql2);
    $row2 =$result2-> fetch_assoc();

    $sql3 = "SELECT * from compbank  where orgid ='$debitnote_comp_code' limit 1";
            ...
            <tbody>
                <tr>
                    ...
                    <td style="border:1px solid #000;padding:0px;">
                        <table width="100%">
                            <tr>
                                <td style="border-bottom:1px solid #000;padding:5px;">Debit Note No#:<b> <?php echo $row['debitnote_id']; ?></b></td> 
                            </tr> 
                            <tr>
                                <td style="border-bottom:1px solid #000;padding:5px;">GRN  No#:<b> <?php echo $row['debitnote_grn_id']; ?></b></td> 
                            </tr> 
                            <tr>
                                <td style="border-bottom:1px solid #000;padding:5px;">
                                    Credit Note Date: <b><?php echo $row['debitnote_paymentdate']; ?></b>
                                </td> 
                            </tr>    
                        ...
                        <tr>
                            <td width="100%" style="border:1px solid #000;padding-top:10px;padding-left:10px;display:none">
                                <p><b>Terms & Conditions</b><br/>
                                    <?php echo $row['debitnote_tc']; ?></p><br/>
                    ...
                    <tbody>
                        <tr>
                            <td width="100%" style="border:1px solid #000;padding-top:10px;padding-left:10px;">
                                <p><br/>
                                    <?php echo $row['debitnote_notes']; ?></p><br/>
