                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <table id="po_reports" class="table table-bordered" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php
                                            if((isset($_GET['st'])&&$_GET['st']!='')||(isset($_GET['end'])&&$_GET['end']!='')||(isset($_GET['vendorwise'])&&$_GET['vendorwise'])||(isset($_GET['statuswise'])&&$_GET['statuswise']!='')){
                                                ...
                                                $sql = "SELECT * from purchaseorders p,vendorprofile v where 1=1 ";
                                                if($_GET['st']!=''){
                                                    if($st==$end){
                                                        $sql.= " and p.po_date='$st' ";   
                                                    }else{
                                                        $sql.=" and (p.po_date BETWEEN '$st' AND '$end') ";   
                                                    }
                                                }
                                                ...
                                                if(isset($_GET['vendorwise'])&&$_GET['vendorwise']!=''){
                                                    ...
                                                    $sql.=" and p.po_vendor='".$_GET['vendorwise']."'";    
                                                }
                                                if(isset($_GET['statuswise'])&&$_GET['statuswise']!=''){

                                                    $sql.=" and p.po_status='".$_GET['statuswise']."'";    
                                                }
                                                $sql.=" and p.po_vendor=v.vendorid;";    
                                            ...
                                            if ($result->num_rows > 0){
                                                ...
                                                <td>'.$row['po_date'].'</td>
                                                <td>'.$row['supname'].'</td>
                                                <td>'.nf(($row['po_value']-gettaxamt_total($row['po_items']))).'</td>
                                                <td>'.nf(gettaxamt_total($row['po_items'])).'</td>
                                                <td>'.nf($row['po_value']).'</td>
                                            </tr>';  
                                                }
                                            }
                                            ?>
...
<script>
            ...
            buttons: [
                {
                    ...
                    customize: function ( win ) {
                        $(win.document.body)
                            .css( 'font-size', '10pt' )
                            .prepend(
                            '<p><img src="<?php echo $baseurl;?>assets/images/crystalLogo.png" style="width:70px;height:60px;" /></p><p class="lead text-center"><b> Purchase Order Reports</b><br/></p>'+printhead+'</div>'
                        );
    ...
    function get_po_reports(){
        ...
        location.href="PurchaseOrderReports.php?st="+st+"&end="+end+"&vendorwise="+vendorwise+"&statuswise="+statuswise;
    }
